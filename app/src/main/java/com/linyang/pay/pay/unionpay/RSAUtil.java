...
public class RSAUtil {
    ...
    private static final String RSA_PADDING_MODE = "RSA";
    ...
    public static PublicKey getPublicKeyPM() {
        ...
        return RSAUtil.generateRSAPublicKey(modulus, publicExponent);
    }
     ...
     * @return true/false
     */
    public static boolean verify(String data, String sign64, String mode) {
        boolean ret = false;
        try {
            if ("01".equals(mode)) {
                String sha1 = RSAUtil.sha1(data.getBytes());
                if (sha1 != null) {
                    ret = RSAUtil.verifyPM(sha1.getBytes(), Base64.decode(sign64, Base64.NO_WRAP));
                }
            } else if ("00".equals(mode)) {
                String sha1 = RSAUtil.sha1(data.getBytes());
                if (sha1 != null) {
                    ret = RSAUtil.verifyProduct(sha1.getBytes(), Base64.decode(sign64, Base64.NO_WRAP));
                }
            }
            return ret;
        } catch (Exception e) {
